...
[Source](https://github.com/Aitherium/awrouter)  ·  [The Aither World](https://aitherium.github.io/)

> **The Aither World** is an operating system for agents — a Linux you can hand to one, the runtimes it works in, and the tools it works with. [awnix](https://github.com/Aitherium/awnix) is the Linux underneath it; **awrouter** is one of its 65 bricks — each installs on its own, runs offline, and needs no account.
>
...
## Load-aware failover (optional)

Hand `resolve()` a `LoadTracker` and the capable set is ordered by live
load instead of the static score: `pending + GPU pressure (0-3, EWMA with
hysteresis, NEUTRAL 1 when telemetry is missing or older than 10 s) + this
process's own in-flight reservations`, then pressure, then score, then
stable id. The winner is reserved before `resolve()` returns, so a burst of
requests spreads across equal backends before any telemetry catches up.
`Resolution.ranked` is the failover list: on a retryable upstream status
(`is_retryable(status, inference=True)` — 408/429/5xx, and 404 only for an
inference call) walk to the next entry; a 4xx client error is not retried
anywhere. Release the reservation when the request ends, `move()` it if
failover landed elsewhere, and `apply_snapshot()` when a fresher pending
view arrives — it supersedes every reservation taken before it.

```python
from awrouter import LoadTracker
tracker = LoadTracker()
tracker.observe_utilization("fast", 72.0)      # from your GPU telemetry feed
res = resolver.resolve("quick-model", load=tracker)
try:
    ...  # dispatch to res.backend; on is_retryable(status) try res.ranked[1:]
finally:
    tracker.release(res.reservation)
```
...
| **awrouter** _(you are here)_ | OpenRouter for your own fleet: pick a model backend by cost/latency/ capability, fail over, fit the context window, stream. Standalone, OpenAI-compatible, no Aither-specifics required to be valuable | — |
...
<div id="aither-constellation" data-self="awrouter"></div>
